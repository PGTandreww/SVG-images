...
function showCoords(evt)
{
	...
	if(applet != null)
	{
		x = getDim(getX(evt),false);
		y = getDim(getY(evt),false);
 ...
 * @return
 */
function getDim(units,showFeet)
{
...
var redrawScript = 	"drawHorizontalBars('"+liteName+"','"+gridElName+"',"+x+","+y+","+w+","+h+","+horz+",'"+color+"',"+t+","+startAtTop+","+true+")";
	...
	{	
		...
		desc = desc.replace(/%W%/g,getDim(w,false));
		desc = desc.replace(/%P%/g,getDim(punchY,false));
		s(ghVarName,punchY);
...
trace("fillV="+fillV);
	...
	{
		var desc = "WIDTH: " + getDim(w,false) + " HEIGHT: " + getDim(h,false) + ".";
...
trace("initCircle('"+id+"',"+x+","+ y+","+ r+",'"+ color+"')");
	...
	{
		var desc = "RADIUS: " + getDim(r,false) + ".";
		createDescription(id,desc);
	}
}	
catch(e)
 ...
 * @param internal      boolean is the dim internal or external to the object it is measuring
 * @return	void
 */ 

function createDim(x1,y1,x2,y2,showFeet,internal)
{
try{
...
//  		line1.setAttribute("y1",y1-.75);
// 		line1.setAttribute("y2",y1+.75);
// 		line2.setAttribute("x1",x2);
//  		line2.setAttribute("x2",x2);
//  		line2.setAttribute("y1",y2-.75);
// 		line2.setAttribute("y2",y2+.75);
// 	}
// 	else
// 	{
//   		line1.setAttribute("x1",x1-.75);
//  		line1.setAttribute("x2",x1+.75);
//  		line1.setAttribute("y1",y1);
// 		line1.setAttribute("y2",y1);
// 		line2.setAttribute("x1",x2-.75);
//  		line2.setAttribute("x2",x2+.75);
...
// create starting arrow  	
	...
	var g2 = drawing.createElementNS(svgNS,"g");  
  	g2.setAttribute("transform","rotate("+rotate+")");
   	var g3= drawing.createElementNS(svgNS,"g");
   	if(internal || length < (um === "mm" ? 200 : 10)){
            g3.setAttribute("transform","scale(0.5 1)");
        }
...
// create ending arrow	
	...
	g2 = drawing.createElementNS(svgNS,"g");  
  	g2.setAttribute("transform","rotate("+rotateRev+")");
   	g3= drawing.createElementNS(svgNS,"g");
   	if(internal || length < (um === "mm" ? 200 : 10)){
            g3.setAttribute("transform","scale(0.5 1)");
        }
...
// create text caption
	...
	text.setAttribute("y",(yC+(internal||length<(um === "mm" ? 200 : 10)?.625:1)));
	text.setAttribute("class",(internal||length<(um === "mm" ? 200 : 10)?"dimSmall":"dim"));
...
function adjustColor(col, amt) {
    ...
    trace("adjusted color:" + (usePound?"#":"") + String("000000" + (g | (b << 8) | (r << 16)).toString(16)).slice(-6));
